package backend.academy.maze;

public class Constants {
    private Constants() {

    }

    public static final int MIN_SIZE = 5;
    public static final int MAX_SIZE = 50;
    public static final int SWAMP_VALUE = 3;
    public static final int SWAMP_DIVIDER = 5;
    public static final String HORIZONTAL_WALL = "---";
    public static final String SPACE = "   ";
    public static final String CROSS_LINE_BREAKER = "+\n";
    public static final String SWAMP_SYMBOL = " # ";
    public static final String WALL_LINE_BREAKER = "|\n";
    public static final String LINE_BREAKER = " \n";
    public static final String BEFORE = " до ";
    public static final String TYPE_X = "Введите x координату от 0 до ";
    public static final String TYPE_Y = "Введите y координату от 0 до ";
    public static final String ERROR_X = "Ошибка: Не удалось прочитать позицию X";
    public static final String ERROR_Y = "Ошибка: Не удалось прочитать позицию Y";
    public static final String ERROR_COORDINATE = "Ошибка: Координата должна быть числом.";
}
